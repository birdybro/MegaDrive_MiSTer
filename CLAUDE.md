...
- **`ym7101.v`** — VDP (Video Display Processor). **Fully annotated** — all opaque `w###`/`l###`/`t###` signals have been renamed to descriptive names (~1750 total). Module doc comment describes architecture (scroll planes, sprites, DMA, VRAM, color RAM, DAC, PSG). Section comments with `// ---...` banners mark 14 functional blocks (prescaler, I/O & DMA, timing FSM, H/V PLAs, scroll planes, VSRAM, sprite processing, SAT/sprdata/linebuffer, VRAM interface, video MUX, DAC, color RAM, PSG, bus drive, misc outputs). Wire/reg declarations have grouped sub-section comments. Helper submodule `ym7101_rs_trig` has a doc comment. Signal naming covers: CPU bus interface (`cpu_wr_strobe`, `bus_active`), interrupt priority (`hint_irq`, `vint_irq`, `eint_pend`), test registers (`tst_fn0_wr`–`tst_fn8_rd`), DMA engine (`dma_copy_mode`, `dma_fill_mode`, `dma_state`, `dma_len_last`), FIFO pipeline (`fifo_wr_ptr`, `fifo_rd_b0/b1`, `fifo_cnt_b0`, `fifo_data_0`–`fifo_data_3`, `fifo_out_uds/lds/cd0-cd3`), register write enables (`reg_wr_strobe`, `wr_en_8F`, `wr_en_93`), timing FSM (`vint_pending`, `vsync_gate`, `disp_active_flag`, `slot3_active`), scroll planes (`vscroll_sum`, `win_zone_active`, `pixel_col_pos`, `planeA_pixel`, `planeB_pixel`, shift registers `sr_a_bp0`–`sr_b_ext3`), sprite engine (`sat_ypos`, `spr_out_pri`, `linebuf_wr_en`), VRAM interface (`vram_wr_timing`, `vram_cas_dly`), video MUX (`sel_spr_final`, `sh_mode_active`), PSG (`psg_freq_0`, `psg_noise_ctrl`, `lfsr_out`), and DAC (`dac_thermo[0:2]`).
